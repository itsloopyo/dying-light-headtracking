...
try {
    ...
    $readme = @(
        ...
        'the Dying Light exe directory as `winmm.dll`. Both `engine_x64_rwdi.dll` and',
        '`gamedll_x64_rwdi.dll` import winmm, and it is not a KnownDLL, so the copy beside',
        'the exe is the one that loads.'
    ) -join "`n"
    [IO.File]::WriteAllText((Join-Path $vendorAsiDir 'README.md'), $readme + "`n", (New-Object System.Text.UTF8Encoding $false))
