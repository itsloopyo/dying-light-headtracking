// Runs core's canonical config lint over DyingLightHeadTracking.ini, the committed file, and
...
const committed = path.join(repo, "DyingLightHeadTracking.ini");
for (const problem of lintCanonicalConfig(fs.readFileSync(committed), options)) {
  failures.push(`DyingLightHeadTracking.ini: ${problem}`);
}
